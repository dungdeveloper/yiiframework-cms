...
class LanguagesController extends AdminBaseController {
    ...
    public function actiontranslate() {
        ...
        $id = Yii::app()->request->getParam('id', 0);
        $cat = Yii::app()->request->getParam('cat', 0);
        ...
        if ($cat !== 0) {
            $criteria->condition .= ' AND id IN (SELECT id FROM sourcemessage WHERE category=:cat)';
            $criteria->params[':cat'] = $cat;
            $count = Message::model()->count('language=:lang AND id IN (SELECT id FROM sourcemessage WHERE category=:cat)', array(':lang' => $id, ':cat' => $cat));
        }
        else {
